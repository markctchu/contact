      ...
      <div className="grid md:grid-cols-2 gap-6 sm:gap-12 w-full min-h-0 overflow-hidden">
        <section className="bg-gray-800 p-6 sm:p-8 rounded-2xl shadow-xl border border-gray-700 flex flex-col shrink-0">
          ...
          <form onSubmit={(e) => {
            ...
            <input
              ...
              className="w-full bg-gray-900 border border-gray-700 rounded-xl px-4 py-3 mb-4 focus:outline-none focus:ring-2 focus:ring-blue-500 text-white transition-all text-sm sm:text-base"
            />
            <button
              type="submit"
              className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl transition-all shadow-lg active:transform active:scale-95 text-sm sm:text-base"
            >
              Launch Room
            </button>
          </form>
        ...
        <section className="bg-gray-800 p-6 sm:p-8 rounded-2xl shadow-xl border border-gray-700 flex flex-col min-h-0 overflow-hidden">
          ...
          <div className="space-y-3 flex-1 overflow-y-auto pr-2 custom-scrollbar min-h-0">
            {rooms.length === 0 ? (
              <p className="text-gray-500 text-center py-8 italic text-sm sm:text-base">No active rooms found. Start one!</p>
            ) : (
              rooms.map((room) => (
                <div
                  key={room.id}
                ...
                >
                  <div className="min-w-0 flex-1 mr-2">
                    <h3 className="font-bold text-white group-hover:text-blue-400 transition-colors text-sm sm:text-base truncate">{room.name}</h3>
                    <p className="text-[10px] sm:text-xs text-gray-500 truncate">{room.playerCount} Players • {room.status}</p>
                  </div>
                  <button
                    onClick={() => onJoinRoom(room.id)}
                    className="bg-gray-800 hover:bg-blue-600 text-white px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-bold transition-all shadow-sm group-hover:shadow-blue-900 shrink-0"
                  >
                    Join
                  </button>
                </div>
